...
export default function DinerDashboard(props: Props) {
  ...
  async function updateUser() {
    ...
    const serviceUser = await pizzaService.updateUser(updatedUser);
    props.setUser(serviceUser);
    setTimeout(() => {
        ...
        <div className="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto min-h-[calc(100%-3.5rem)]">
          <div className="w-full flex flex-col bg-white border shadow-sm rounded-xl pointer-events-auto">
            <div className="flex justify-between items-center py-3 px-4 border-b bg-slate-200 rounded-t-xl">
              <h3 id="dialog-title" className="font-bold text-gray-800">
                Edit user
              </h3>
            ...
            <div className="p-4 overflow-y-scroll max-h-52">
              <div className="my-4 text-lg text-start grid grid-cols-5 gap-2 items-center">
                <label htmlFor="edit-user-name" className="font-semibold">
                  name:
                </label>
                <input id="edit-user-name" aria-label="name" type="text" className="col-span-4 border border-gray-300 rounded-md p-1" defaultValue={user.name} ref={nameRef} />
                <label htmlFor="edit-user-email" className="font-semibold">
                  email:
                </label>
                <input id="edit-user-email" aria-label="email" type="email" className="col-span-4 border border-gray-300 rounded-md p-1" defaultValue={user.email} ref={emailRef} />
                <label htmlFor="edit-user-password" className="font-semibold">
                  password:
                </label>
                <input id="edit-user-password" aria-label="password" type="text" className="col-span-4 border border-gray-300 rounded-md p-1" defaultValue="" ref={passwordRef} />
              </div>
            </div>
